export const changesToOrderSticker = (
    order,
    skipped = false,
    ...
    const orderChanges = getOrderChangesSticker(order, skipped, orderPreparationCategories);
 ...
 */
export const getOrderChangesSticker = (order, skipped = false, orderPreparationCategories) => {
    ...
    for (const orderline of order.get_orderlines()) {
        ...
        const sticker_note = orderline.get_sticker_note();
        ...
        if (prepaCategoryIds.size === 0 || productCategoryIds.length > 0) {
            ...
            if (quantityDiff && orderline.skip_change === skipped) {
                ...
                if (oldChanges[relatedKey] && (note || sticker_note)) {
        ...
        if (!order.models["pos.order.line"].getBy("uuid", lineResume["uuid"])) {
            ...
            if (!changes[lineKey]) {
                changes[lineKey] = {
                    ...
                    sticker_note: lineResume["sticker_note"],
